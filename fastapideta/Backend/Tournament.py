...
def All_Tournament(db:Session):
    tournaments = db.query(models.Tournament, models.User, models.Game).join(models.User, models.Tournament.coach == models.User.usn).join(models.Game, models.Tournament.game == models.Game.g_id).all()
...
def Find_Tournament(id:int, db:Session):
    t = db.query(models.Tournament, models.User, models.Game).join(models.User, models.Tournament.coach == models.User.usn).join(models.Game, models.Tournament.game == models.Game.g_id).filter(models.Tournament.t_id == id).first()
...
def Find_Registration(id:int, db:Session):
    register = db.query(models.User, models.Registration).join(models.Registration, models.Registration.r_player == models.User.usn).filter(models.Registration.r_tour == id).all()
    result = [Schema.Profile(name=r[0].name, usn=r[0].usn, email=r[0].email, dob=str(r[0].dob), role=r[0].role) for r in register]
    return result
